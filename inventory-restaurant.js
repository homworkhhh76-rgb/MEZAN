(function(){
  ...
  function ensureDB(){
    ...
    if(!d.accounts.find(a=>a.id==='ACC-5230'))d.accounts.push({id:'ACC-5230',code:'5230',name:'مصروفات التوالف والهالك',type:'expense',parentId:'ACC-5000',level:1,balance:0});
    if(!d.accounts.find(a=>a.id==='ACC-1400'))d.accounts.push({id:'ACC-1400',code:'1400',name:'إنتاج تحت التشغيل',type:'asset',balance:0});
  ...
  function addBatchModal(){
    const d=D();
    A.openModal({title:'إضافة تشغيلة',body:`<div class="form-grid"><label class="field"><span>الصنف</span><select name="productId" required>${A.options(d.products.filter(p=>p.active!==false),'id','name')}</select></label><label class="field"><span>المستودع</span><select name="warehouseId" required>${A.options(d.warehouses.filter(w=>w.active),'id','name',S.activeWarehouseId||d.warehouses[0]?.id)}</select></label><label class="field"><span>رقم التشغيلة</span><input name="batchNo" required></label><label class="field"><span>تاريخ الانتهاء</span><input type="date" name="expiryDate"></label><label class="field"><span>الكمية الأساسية</span><input type="number" step="any" min="0.00000001" name="qtyBase" required></label><label class="field"><span>تكلفة الوحدة</span><input type="number" step="0.01" min="0" name="cost"></label><label class="field full"><span>ملاحظات</span><textarea name="notes"></textarea></label></div>`,onSubmit:fd=>{const p=findProduct(fd.get('productId'));if(!p)return;const lot={id:A.uid('BAT'),productId:p.id,warehouseId:fd.get('warehouseId'),batchNo:fd.get('batchNo').trim(),expiryDate:fd.get('expiryDate'),qtyBase:num(fd.get('qtyBase')),cost:num(fd.get('cost')||p.cost||0),notes:fd.get('notes').trim(),createdAt:A.now()};D().productBatches.unshift(lot);A.adjustStock(lot.productId,lot.warehouseId,lot.qtyBase);A.saveDB();A.audit('إضافة تشغيلة','المخزون',`${p.name} - ${lot.batchNo}`);A.renderCurrent();A.toast('تمت إضافة التشغيلة');}})
  }
