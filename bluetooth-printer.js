(function(){
  ...
  async function printInvoice(sale){
    if(!sale)throw new Error('الفاتورة غير موجودة.');const s=settings();if(s.printSize==='a4'||s.printerConnection!=='bluetooth'){window.open(`./index.html?v=740#print-invoice/${sale.id}`,'_blank');return false}
    if(!support()){A.toast?.('الطباعة بالبلوتوث غير مدعومة في هذا المتصفح؛ سيتم فتح الطباعة العادية.','warning',5000);window.open(`./index.html?v=740#print-invoice/${sale.id}`,'_blank');return false}
    state.printing=true;emit();try{if(!state.characteristic||!state.device?.gatt?.connected)await connectSaved();const canvas=await receiptCanvas(sale),bytes=rasterBytes(canvas);await writeBytes(bytes);A.toast?.(`تمت طباعة ${sale.number} على ${state.device?.name||'طابعة البلوتوث'}`,'success');return true}catch(e){console.error(e);A.toast?.(`${e.message||e} — تم فتح الطباعة العادية كبديل.`,'warning',6000);window.open(`./index.html?v=740#print-invoice/${sale.id}`,'_blank');return false}finally{state.printing=false;emit()}
  }
