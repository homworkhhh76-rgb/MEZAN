...
function availableText(n){return round(n).toLocaleString('ar-EG',{minimumFractionDigits:2,maximumFractionDigits:2})}
...
function transferLine757(line={}){const p=transferProducts().find(x=>x.id===line.productId)||transferProducts()[0];return `<div class="v757-transfer-line"><label class="field v757-product-field"><span>الصنف / المتغير</span><select class="v757-product-select searchable-select">${productOptions757(p?.id||'')}</select></label><div class="v757-line-units">${unitInputs757(p,line)}</div><button type="button" class="icon-button v757-remove-line" title="حذف">${I('trash',16)}</button></div>`}
...
function summaryValue(label,value){if(typeof value!=='number')return esc(value);if(/كمية|عدد|أيام|ساعات|نسبة|%/i.test(label))return esc(Number(value.toFixed(3)).toLocaleString('ar-EG'));return A.money(value)}
...
.v757-transfer-line{display:grid;grid-template-columns:minmax(210px,1.2fr) minmax(320px,2fr) 42px;gap:10px;align-items:end;padding:10px 0;border-bottom:1px solid var(--border,#dbe3ec)}
.v757-unit-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:7px}.v757-unit-field{display:flex;flex-direction:column;gap:4px;font-size:11px;color:var(--muted,#64748b)}.v757-unit-field input{width:100%;min-height:40px;border:1px solid var(--border,#cbd5e1);border-radius:9px;padding:7px 9px;background:var(--surface,#fff);color:inherit}.v757-receive-line{display:grid;grid-template-columns:minmax(180px,.8fr) minmax(320px,2fr);gap:12px;align-items:center;padding:10px;border-bottom:1px solid var(--border,#dbe3ec)}.v757-receive-line small{display:block;margin-top:3px}@media(max-width:760px){.v757-transfer-line,.v757-receive-line{grid-template-columns:1fr}.v757-transfer-line>.icon-button{justify-self:start}.v757-unit-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
