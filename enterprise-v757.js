...
function printTransfer757(kind,id,forceA4=false){
 const x=transferDoc(kind,id);if(!x)return;const d=D(),st=d.settings||{},size=forceA4?'a4':(st.printSize||'80'),thermal=size!=='a4',paper=size==='58'?'54mm':size==='80'?'76mm':'auto',title=transferTitle(kind,x),rows=transferRows757(x),logo=st.showInvoiceLogo!==false?(A.systemLogoSrc?.()||new URL('brand-logo.png',location.href).href):'';
...
A.exportTablePDF=async function(btn,opts={}){
 try{
  const r=A.tableExportData(btn);if(opts.title)r.title=opts.title;const d=D(),logo=(A.systemLogoSrc?.()||new URL('brand-logo.png',location.href).href),w=window.open('','_blank');if(!w)throw Error('اسمح بفتح النوافذ لتجهيز PDF.');
