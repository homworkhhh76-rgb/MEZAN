/* Al-Meezan Pro v7.81.5 — robust cashier live search fix */
(()=>{
'use strict';
...
function apply(value=''){
 if(S.view!=='cashier')return 0;const root=document.getElementById('workspace');if(!root)return 0;const raw=String(value??''),q=norm(raw),tokens=q.split(' ').filter(Boolean);S.cashierSearch=raw;if(q&&S.cashierCategory&&S.cashierCategory!=='all'&&!S._v7815SearchRerender){S.cashierCategory='all';S._v7815SearchRerender=true;const activeId=document.activeElement?.id||'cashierSearch';setTimeout(()=>{try{A.renderCurrent('cashier')}finally{S._v7815SearchRerender=false}requestAnimationFrame(()=>{const n=document.getElementById(activeId)||document.querySelector(ids);if(n){n.value=raw;try{n.focus({preventScroll:true})}catch(_){n.focus()}}apply(raw)})},0);return 0}if(q)showCatalog(root);let shown=0;
...
function handle(e){const input=e.target;if(S.view!=='cashier'||!input?.matches?.(ids))return;const raw=input.value||'';S.cashierSearch=raw;if(raw.trim()&&S.cashierCategory&&S.cashierCategory!=='all'){
   S.cashierCategory='all';const inputId=input.id;A.renderCurrent('cashier');requestAnimationFrame(()=>{const n=document.getElementById(inputId)||document.querySelector(ids);if(n){n.value=raw;try{n.focus({preventScroll:true})}catch(_){n.focus()}}apply(raw)});return
 }
 apply(raw)
}
...
const ws=document.getElementById('workspace');if(ws)new MutationObserver(()=>{if(S.view==='cashier')requestAnimationFrame(()=>{A.cashierApplySearch=apply;if(S.cashierSearch)apply(S.cashierSearch)})}).observe(ws,{childList:true,subtree:true});
})();
